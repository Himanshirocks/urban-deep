...
class DQN_Agent():
	...
	def epsilon_greedy_policy(self, q_values, train_test_var):
		...
		#Different Epsilons for fitting model and evaluating the training
		if train_test_var:
		...
		for i_episode in range(self.episodes):
			...
			for t_iter in range(self.iterations):
				...
				if self.environment_num == 1:
					...
					if total_updates<=100000:
						self.train_epsilon = -((starting_epsilon-0.1)/100000)*total_updates + starting_epsilon
					else:
				...
				if done:
					...
					total_updates+=1
					if (self.environment_num==1 and total_reward>-200) or (self.environment_num==2 and t_iter+1==199):
						success_count+=1
					if (total_reward > max_reward):
						max_reward = total_reward
					print("Episode {} total reward {} epsilon {} successes {} total_updates {} Max_Reward {} "
																.format(i_episode,total_reward,self.train_epsilon,success_count, total_updates, max_reward))
	...
	def test(self, model_file=None):
		...
		for i in range(1,self.final_update/10000):
			...
			plt.title('%d Model' %(i))
			# plt.show()			
			plt.savefig('graph_%d_%d.eps' %(self.environment_num,i*10000),bbox_inches='tight' )
